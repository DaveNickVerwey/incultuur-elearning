...
function Welkom() {
  ...
  return (
    <div style={{ minHeight: '100vh', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{
        position: 'fixed',
        inset: 0,
        backgroundImage: 'url(https://images.unsplash.com/photo-1507676184212-d03ab07a01bf?w=1600)',
      ...
      {/* Inhoud */}
      <div style={{ position: 'relative', zIndex: 1, background: 'white', borderRadius: '16px', padding: '3rem', maxWidth: '640px', width: '100%', margin: '2rem', boxShadow: '0 8px 40px rgba(0,0,0,0.12)' }}>

        <div style={{ width: '40px', height: '40px', background: blauw, borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 'bold', marginBottom: '2rem' }}>
  {profiel ? `${profiel.naam.charAt(0)}${profiel.achternaam.charAt(0)}`.toUpperCase() : 'IC'}
</div>

        <h1 style={{ color: blauw, fontSize: '1.75rem', margin: '0 0 0.5rem' }}>
          Hi {profiel.naam}, tof dat je erbij bent! 👋
        </h1>

        <p style={{ color: groen, fontWeight: '600', fontSize: '1rem', marginBottom: '1.5rem' }}>
        ...
        <div style={{ color: '#444', lineHeight: '1.7', fontSize: '0.95rem' }}>
          <p>
            Toegankelijkheid gaat over meer dan een rolstoelhellingbaan bij de ingang. Het gaat over de vraag wie zich welkom voelt in jouw organisatie — en wie niet. Over drempels die je ziet, maar ook over drempels die je niet ziet.
          </p>
          <p>
          ...
          <div style={{ background: '#f8f9ff', border: `1px solid ${blauw}20`, borderRadius: '12px', padding: '1.5rem', margin: '1.5rem 0' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {[
                ...
                <div key={stap.nr} style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
                  <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: groen, color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.8rem', fontWeight: 'bold', flexShrink: 0 }}>{stap.nr}</div>
                  <p style={{ margin: 0, paddingTop: '0.3rem', color: '#444' }}>{stap.tekst}</p>
                </div>
              ))}
            </div>
          </div>
  ...
  <iframe
    width="100%"
    height="315"
    src="https://www.youtube.com/embed/3qWs7rInuiQ"
    title="InCultuur introductievideo"
    frameBorder="0"
...
<p style={{ color: '#888', fontSize: '0.9rem' }}>
  Je voortgang wordt automatisch opgeslagen. Je kunt de e-learning op elk moment pauzeren en later verder gaan.
        ...
        <button
          ...
          style={{ marginTop: '2rem', width: '100%', padding: '1rem', background: blauw, color: 'white', border: 'none', borderRadius: '10px', fontSize: '1rem', fontWeight: '600', cursor: 'pointer' }}
        >
          Naar mijn dashboard →
        </button>
      </div>
